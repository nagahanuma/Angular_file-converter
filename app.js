...
app.get('/pdftoword',function(req,res){
  res.sendFile(__dirname+'/pdftoword.html');
})
app.post('/upload',function(req,res){
  ...
  if(req.files.upfile){
    ...
    var uploadpath = __dirname + '/build/uploads/' + name;
    ...
    file.mv(uploadpath,function(err){
      if(err){
        ...
        var initialPath = path.join(__dirname, `./build/uploads/${First_name}${extend_docx}`);
        //Path where the converted pdf will be placed/uploaded
        var upload_path = path.join(__dirname, `./build/uploads/${First_name}${extend_pdf}`);
        ...
        res.sendFile(__dirname+'/build/down_html.html')
        });
      }
    });
  }else{
...
app.get('/download', (req,res) =>{
  ...
  res.download(__dirname +`/build/uploads/${down_name}${extend_pdf}`,`${down_name}${extend_pdf}`,(err) =>{
    if(err){
      res.send(err);
    }else{
      //Delete the files from directory after the use
      console.log('Files deleted');
      const delete_path_doc = process.cwd() + `/build/uploads/${down_name}${extend_docx}`;
      const delete_path_pdf = process.cwd() + `/build/uploads/${down_name}${extend_pdf}`;
      try {
...
app.get('/thankyou',(req,res) => {
    res.sendFile(__dirname+'/build/thankyou.html')
})
